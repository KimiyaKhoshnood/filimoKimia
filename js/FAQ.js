const FAQ = async () => {
  axios.get("http://localhost:3004/FAQ").then((res) => {
    ...
    for (let i in res.data.FAQs) {
      let word = res.data.FAQs[i][1];
      if (res.data.FAQs[i][2] !== false) {
        for (let j in res.data.FAQs[i][2]) {
          word = word.replace(
            res.data.FAQs[i][2][j],
            `<span class="text-[#FDC13C]">${res.data.FAQs[i][2][j]}</span>`
          );
        }
      }
    ...
    let FAQtopic = document.querySelectorAll(
      "#FAQ>div>div>div:nth-child(1)>div"
    );
    ...
    document
      .querySelector("#dropdownMobile>li")
      .addEventListener("click", () => {
        ...
        let tempFAQmobile = "";
        for (let i in res.data.FAQs) {
          let wordMobile = res.data.FAQs[i][1];
          if (res.data.FAQs[i][2] !== false) {
            for (let j in res.data.FAQs[i][2]) {
        ...
        for (let i in Array.from(FAQboxMobile)) {
